...
public class TestLongArrayIterator {

	private final long[] expected = new long[]{20,30,40,50};
	
	@Test(expected= NullPointerException.class)
	public void nullArrayShouldThrowNPE(){
		PrimitiveArrayIterators.create((long[]) null);
	}
	
	@Test(expected= NullPointerException.class)
	public void nullArrayWithLengthConstructorShouldThrowNPE(){
		PrimitiveArrayIterators.create((long[]) null, 20);
	}
	...
	public void nullArrayWithStartAndEndConstructorShouldThrowNPE(){
		PrimitiveArrayIterators.create((long[]) null, 20,40);
	}
	...
	private void assertIterateCorrectly(Iterator<Long> actual, long...expectedBytes){
		for(int i=0; i< expectedBytes.length; i++){
			assertTrue(actual.hasNext());
			assertEquals(expectedBytes[i], actual.next().longValue());
		}
		try{
			actual.remove();

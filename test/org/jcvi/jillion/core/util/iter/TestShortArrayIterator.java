...
public class TestShortArrayIterator {

	private final short[] expected = new short[]{20,30,40,50};
	
	@Test(expected= NullPointerException.class)
	public void nullArrayShouldThrowNPE(){
		PrimitiveArrayIterators.create((short[]) null);
	}
	
	@Test(expected= NullPointerException.class)
	public void nullArrayWithLengthConstructorShouldThrowNPE(){
		PrimitiveArrayIterators.create((short[]) null, 20);
	}
	...
	public void nullArrayWithStartAndEndConstructorShouldThrowNPE(){
		PrimitiveArrayIterators.create((short[]) null, 20,40);
	}
	...
	private void assertIterateCorrectly(Iterator<Short> actual, short...expectedBytes){
		for(int i=0; i< expectedBytes.length; i++){
			assertTrue(actual.hasNext());
			assertEquals(expectedBytes[i], actual.next().shortValue());
		}
		try{
			actual.remove();

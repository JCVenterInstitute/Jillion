...
public abstract class FastqFileParser implements FastqParser{
	...
	ParserState parseNextRecord(FastqVisitor visitor, LineParser parser, ParserState parserState) throws IOException{
	...
	private class FastqIteratorImpl implements FastqSingleVisitIterator{

		private ParserState parserState;
		private final LineParser parser;
		
		public FastqIteratorImpl( ParserState parserState, LineParser parser) {
		...
		public boolean hasNext() {
			...
			return parserState.keepParsing() && parser.hasNextLine();
		}
		...
		public void next(FastqVisitor visitor) {
			...
			try {
				parseNextRecord(visitor, parser, parserState);
			} catch (IOException e) {
				Sneak.sneakyThrow(e);
			}
		}
	...
	private static class FileBasedFastqFileParser extends FastqFileParser{
		...
		public FastqSingleVisitIterator iterator() throws IOException{
			InputStream in = supplier.get(InputStreamSupplier.InputStreamReadOptions.builder()
					.nestedDecompress(true)
					.build());
			LineParser parser = trackPosition? new TextLineParser(in) :new PositionlessLineParser(in);
			ParserState parserState = trackPosition ? new ParserState(parser.getPosition()) : new ParserState(0);
			
			return new FastqIteratorImpl(parserState, parser);
		}
		@Override
		public FastqSingleVisitIterator iterator(FastqVisitorMemento memento) throws IOException{
			...
			InputStream in = supplier.get(InputStreamSupplier.InputStreamReadOptions.builder()
					...
					.start(startOffset)
					.build());
			LineParser parser = new TextLineParser(in, startOffset);
			ParserState parserState = new ParserState(startOffset);
	...
	private static class InputStreamFastqFileParser extends FastqFileParser{
		...
		public FastqSingleVisitIterator iterator() throws IOException{
			...
			LineParser parser = new PositionlessLineParser(in);
			ParserState parserState = new ParserState(0);

...
final class DefaultReferenceEncodedNucleotideSequence extends AbstractResidueSequence<Nucleotide, NucleotideSequence, NucleotideSequenceBuilder>
	...
	/**
	 * All of the differences between this read and the reference it is aligned
	 * to (which could be a contig consensus) are encoded here. If there are no
	 * SNPs in this read: meaning that this read aligns to its reference at 100%
	 * identity, then this value <strong> will be null</strong>. <br/>
	 * The encoding uses {@link ValueSizeStrategy} to pack the data in as few
	 * bytes as possible. Here is the current encoding:<br/>
	 * <ul>
	 * <li>first byte is the ordinal value of the {@link ValueSizeStrategy} used
	 * to store the number of elements</li>
	 * 
	 ...
	 * used to store the number of bytes required for each SNP offset value</li>
	 * 
	 * <li>the next $num_snps * 1,2 or 4 bytes (depending on the
	 * {@link ValueSizeStrategy} specified in the previous byte) encode the SNP
	 * offsets in the read.</li>
	 * 
	 * </li> the remaining bytes store the actual SNP values as Nucleotide
	 * ordinal values packed as 4 bits each. This means that each byte actually
	 * stores 2 SNPs.</li>
	 * <ul/>
	 */
	private final byte[] encodedSnpsInfo;
	/**
	 * Our HashCode value, This value is lazy loaded so we only have to compute
	 * the hashcode value once.
	 * 
	 * We can afford to store it because the Java memory model will padd out the
	 * bytes anyway so we don't take up any extra memory.
	 */
	...
	public SortedMap<Integer, Nucleotide> getDifferenceMap() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (i % 2 == 0) {
				...
				differenceMap.put(offset,
						Nucleotide.VALUES.get(snps[index] & 0x0F));
			}
	...
	public DefaultReferenceEncodedNucleotideSequence(
			NucleotideSequence reference, String toBeEncoded, int startOffset) {
	...
	public DefaultReferenceEncodedNucleotideSequence(
		...
		SortedMap<Integer, Nucleotide> differentGlyphMap = populateFields(
				reference, toBeEncoded, startOffset);
		...
		ValueSizeStrategy snpSizeStrategy = ValueSizeStrategy
				.getStrategyFor(differentGlyphMap.lastKey().intValue());
		int snpByteLength = computeNumberOfBytesToStore(numSnps,
				snpSizeStrategy);
		ValueSizeStrategy numSnpsStrategy = ValueSizeStrategy
				.getStrategyFor(snpByteLength);
		int bufferSize = numSnpsStrategy.getNumberOfBytesPerValue()
				+ snpByteLength;
	...
	public Stream<Range> findMatches(Pattern pattern) {
		...
		return StreamUtil.newGeneratedStream(() -> matcher.find() ? Optional
				.of(Range.of(matcher.start(), matcher.end() - 1)) : Optional
				.empty());
	}
	...
	public Stream<Range> findMatches(Pattern pattern, Range subSequenceRange) {
		...
		return StreamUtil.newGeneratedStream(() -> matcher.find() ? Optional
				.of(new Range.Builder(matcher.start(), matcher.end() - 1)
						.shift(subSequenceRange.getBegin()).build()) : Optional
				.empty());
	}
	...
	private int computeNumberOfBytesToStore(int numSnps,
			ValueSizeStrategy snpSizeStrategy) {
	...
	private SortedMap<Integer, Nucleotide> populateFields(
		...
		Iterator<Nucleotide> refIterator = reference
				.iterator(new Range.Builder(length).shift(startOffset).build());
		int i = 0;
	...
	private void handleAfterReference(Sequence<Nucleotide> reference,
		...
		if (lastOffsetOfSequence > reference.getLength()) {
			int overhang = (int) (toBeEncoded.getLength() + startOffset - reference
					.getLength());
			throw new IllegalArgumentException(String.format(
					"sequences extends beyond reference by %d base(s)",
					overhang));
		}
	}
	...
	private void handleBeforeReference(int startOffset) {
		if (startOffset < 0) {
			...
			throw new IllegalArgumentException(
					"can not start before reference: " + startOffset);
		}
	}
	...
	private Nucleotide[] asNucleotideArray(Range range) {
		...
		if (range.getEnd() >= length) {
			throw new IndexOutOfBoundsException(range.getEnd()
					+ " is beyond end of sequence (length = " + length + ")");
		}
		Nucleotide[] array = asNucleotideArray();

		return Arrays.copyOfRange(array, (int) range.getBegin(),
				(int) range.getEnd() + 1);
	}
	...
	private Nucleotide[] asNucleotideArray() {
		...
		if (encodedSnpsInfo != null) {
			...
			ValueSizeStrategy numSnpsSizeStrategy = ValueSizeStrategy.values()[buf
					.get()];
			int size = numSnpsSizeStrategy.getNext(buf);
			ValueSizeStrategy sizeStrategy = ValueSizeStrategy.values()[buf
					.get()];
			...
			for (int i = 0; i < size; i++) {
				...
				int snpIndex = i / 2;
				if (i % 2 == 0) {
					array[index] = Nucleotide.VALUES
							.get(snps[snpIndex] >> 4 & 0x0F);
				} else {
	...
	private byte[] getSnpArray(ValueSizeStrategy numSnpsSizeStrategy, int size,
			ValueSizeStrategy sizeStrategy) {
		int from = numSnpsSizeStrategy.getNumberOfBytesPerValue() + 2 + size
				* sizeStrategy.getNumberOfBytesPerValue();
		return Arrays
				.copyOfRange(encodedSnpsInfo, from, encodedSnpsInfo.length);
	}
	...
	private List<Integer> modifyForSnps(GrowableIntArray gaps) {
		...
		if (buf.hasRemaining()) {
			...
			byte[] snpArray = Arrays.copyOfRange(encodedSnpsInfo,
					encodedSnpsInfo.length - numBytesRemaining,
					encodedSnpsInfo.length);
			for (int i = 0; i < size; i++) {
				int snpIndex = i / 2;
	...
	/**
	 * Most reference gaps should also be present in our gapped sequence so we
	 * need to get the reference gaps that overlap with our sequence range and
	 * shift them accordingly so read coordinate space. (offset 0 is first base
	 * in our read)
	 * 
	 * @return
	 */
	...
	public NucleotideSequenceBuilder toBuilder() {
		return new NucleotideSequenceBuilder(this).setReferenceHint(reference,
				startOffset);
	}

	@Override
	public NucleotideSequenceBuilder toBuilder(Range range) {
		return new NucleotideSequenceBuilder(this, range).setReferenceHint(
				reference, (int) (startOffset + range.getBegin()));
	}
	...
	public List<Range> getRangesOfNs() {
		...
		// for now just do the good not optimal way of looping through
		// eveything.

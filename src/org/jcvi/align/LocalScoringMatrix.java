 ...
 */
public class LocalScoringMatrix<G extends Glyph> implements ScoringMatrix<G>
{
    ...
    private static final int SCORE_SHIFT = 32 - LocalScoringMatrix.PATH_OFFSET;
     ...
     * gap transitions).
     */
    public LocalScoringMatrix(EncodedGlyphs<G> refSequence, EncodedGlyphs<G> querySequence, GapPenalty gapPenalty)
    {
        super();
    ...
    protected void init(int rowCount, int colCount,GapPenalty gapPenalty)
    {
        ...
        for(int row = 0; row < this.rowCount; row++)
        {
            ...
            {
                this.setScore(row, col, Integer.MIN_VALUE,Path.STOP);
            }
        }
       ...
       /* for (int i = 1; i < colCount; i++)
        {
            this.setScore(rowCount-1, i, gapPenalty.getNextGapPenalty(), Path.STOP);
        }
        gapPenalty.reset();
        /*
       ...
       /* for (int i = 1; i < rowCount; i++)
        {
            this.setScore(i, colCount-1, gapPenalty.getNextGapPenalty(), Path.STOP);
        }
        gapPenalty.reset();
        */
    ...
    protected ScoringMatrixElement createScoringMatrixElement(Path path, int score){
        return new DefaultScoringMatrixElement(path, Math.max(0, score));
    }
    ...
    {
        int initialBestScoreIndex = getInitialBestScoreIndex(this.scores);
        this.bestScoreIndex = initialBestScoreIndex;
        for (int row : createRowIterable(this.rowCount))
        {
            for (int col :createColumnIterable(this.colCount))
            {
                final G queryBase = this.refSeq.get(row);
                final G refBase = this.querySeq.get(col);
                 ...
                 */
                final int bestScore = this.getScore(this.bestScoreIndex);
                int currentBestScoreIndex=bestScoreIndex;
                if (isNewBestScore(cellScore, bestScore))
                {
                    currentBestScoreIndex = this.indexOf(row, col);
                }
                bestScoreIndex= currentBestScoreIndex;
            }
           // System.out.println("bestScoreIndex =" + this.bestScoreIndex);
        }
    }
    ...
    public int resolve(int row, int col, int score)
    {
        ...
        final int vertScore = currentGapPenalty + this.getScore(this.indexOf(row + 1, col));
        final int horzScore = currentGapPenalty + this.getScore(row, col + 1);
    ...
    {
        out.print("   | ");
        for(int col = 0; col < this.colCount-1; col++)
        {
        ...
        for(int row = 0; row < this.rowCount; row++)
        {
            out.printf("%2s | ", row<rowCount-1?refSeq.get(row):"$");
            for(int col = 0; col < this.colCount; col++)
            {

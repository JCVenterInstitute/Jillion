...
public class Cas2Consed3 {
	...
	Cas2Consed3(File casFile, File consedOutputDir){
		this.casFile=casFile;
		this.consedOutputDir = consedOutputDir;
	}
	
	public void convert() throws IOException, DataStoreException{
		TrimDataStore trimDatastore = TrimDataStoreUtil.EMPTY_DATASTORE;
		...
		editDir.mkdir();
		File phdDir = new File(consedOutputDir, "phd_dir");
		phdDir.mkdir();
         ...
         NumberOfReadsVisitor numberOfReadsVisitor = new NumberOfReadsVisitor();
         ...
         final Map<String, Phd> phdMap = new HashMap<String, Phd>((int)numberOfReadsVisitor.getReadCounter());
         
         final Map<Integer, DefaultAceContig.Builder> builders = new HashMap<Integer, DefaultAceContig.Builder>();
         final OutputStream phdOut = new FileOutputStream(new File(phdDir, "phd.ball"));
         CasPhdReadVisitor visitor = new CasPhdReadVisitor(
        		 casWorkingDirectory,trimToUntrimmedMap,
        		 FastQQualityCodec.ILLUMINA,gappedReferenceMap.asList(),
        		 trimDatastore,
        		 new DateTime()
        		 ) {
			...
			protected void visitAcePlacedRead(AcePlacedRead acePlacedRead, Phd phd,
				...
				phdMap.put(acePlacedRead.getId(), phd);
         ...
         PhdDataStore phdDataStore = new PhdDataStoreAdapter(
        		 new SimpleDataStore<Phd>(phdMap));
		...
		OutputStream tempOut = new FileOutputStream(tempAce);
         ...
         File ace = new File(editDir, "cas2consed3.ace.1");
	...
	private static class NumberOfReadsVisitor extends AbstractOnePassCasFileVisitor{
